	...
	if(need_auffahrt) {
		...
		if(besch->get_waytype() != powerline_wt) {
			...
			if(  !gr->weg_erweitern( besch->get_waytype(), ribi )  ) {
				...
				weg = weg_t::alloc( besch->get_waytype() );
				weg->set_besch( weg_besch );
				spieler_t::add_construction_costs( sp, -gr->neuen_weg_bauen( weg, ribi, sp ) -weg->get_besch()->get_preis(), end.get_2d(), weg->get_waytype());
			}
			gr->calc_bild();
		}
		else {
			...
			if(  lt==NULL  ) {
				...
				spieler_t::add_construction_costs(sp, -weg_besch->get_preis(), gr->get_pos().get_2d(), weg->get_waytype());
				gr->obj_add(lt);
...
void brueckenbauer_t::baue_auffahrt(karte_t* welt, spieler_t* sp, koord3d end, koord zv, const bruecke_besch_t* besch)
{
	...
	if(besch->get_waytype() != powerline_wt) {
		if(  !bruecke->weg_erweitern( besch->get_waytype(), ribi_neu)  ) {
			// needs still one
			weg = weg_t::alloc( besch->get_waytype() );
			spieler_t::add_construction_costs(sp, -bruecke->neuen_weg_bauen( weg, ribi_neu, sp ), end.get_2d(), besch->get_waytype());
		}
