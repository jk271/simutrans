	...
	for(uint32 i=0; i<get_count(); i++) {
		...
		if(gr==NULL) {
			...
			if(  !keep_existing_faster_ways  ||  (tunnel->get_besch()->get_topspeed() < tunnel_besch->get_topspeed())  ) {
				spieler_t::add_maintenance(sp, -tunnel->get_besch()->get_wartung(), tunnel->get_finance_waytype());
				spieler_t::add_maintenance(sp,  tunnel_besch->get_wartung(), tunnel->get_finance_waytype() );
...
//DBG_MESSAGE("wegbauer_t::baue_strasse()","nothing to do at (%i,%i)",k.x,k.y);
				...
				spieler_t::add_maintenance(s, -weg->get_besch()->get_wartung(), weg->get_finance_waytype());
