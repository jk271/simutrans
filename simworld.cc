...
//				if(!ist_in_kartengrenzen(tmp+koord(2,2))  ||  !ist_in_kartengrenzen(tmp+koord(-2,-2))){ // river ends near map margin
				if(!is_within_limits(tmp+koord(2,2))  ||  !is_within_limits(tmp+koord(-2,-2))){ // river ends near map margin
	...
	//	assert(dig_k != next_dig_k);
		if(dig_k == next_dig_k) { // something may be wrong
			if( valley_coord.get_count() != 1) // check it
			{
				...
				printf("assertion failed; map=%i; gw=%i; mh=%f; roughness=%f %s:%i\n", settings.get_karte_nummer(), settings.get_grundwasser(), settings.get_max_mountain_height(), settings.get_map_roughness(), __FILE__, __LINE__); // double ground fails here 20140122
...
void karte_t::create_valleys()
{
	...
	printf("starting valley correction, map %i, gw=%i; mh=%f; roughness=%f\n", settings.get_karte_nummer(), settings.get_grundwasser(), settings.get_max_mountain_height(), settings.get_map_roughness());
	...
	for(int i=0; i<levels-10;  ++i){  // why 10 ??
		...
		for(int z_detailed = 2; (current_step[i].get_count() > 0 || next_step[i].get_count() > 0)  &&  nobreak2; ++z_detailed) {
			...
			sint8 height = i + get_grundwasser() + 1; // height of land in current level
...
//						lookup_kartenboden(next_k)->set_text(tmp_string);
					...
					// dig a valley / plan a valley
						...
						if (lookup_valley_koords(tmp_world, size_x, valley_coord, k, height))
						{
